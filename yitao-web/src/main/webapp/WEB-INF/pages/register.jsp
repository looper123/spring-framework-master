                                        ...
                                        <div class="step-content row-fluid position-relative" id="step-container">
                                            <div class="step-pane active" id="step1">
                                                ...
                                                <form id="uploadForm" method="post" enctype="multipart/form-data" >
                                                    ...
                                                    标识 <input type="text"  name="imgType"/>
                                                    cid <input type="text"  name="origin"/>
                                                <%--<input type="submit" value="上传">--%>
                                                    <input type="button" value="上传" onclick="fileUpload()">
                                                </form>
                                                <%--实现图片上传后马上回显 原理：在本地建一个.png空文件 onchange时间监控浏览器的上传图片事件
                                                 触发readFile方法 把上传文件流写入到 本地的.png文件中
                                                 再去读取即可
                                                --%>
                                                <h2>上传图片回显</h2>
                                                <form>
                                                    <img id="headimg" src="${pageContext.request.contextPath}/resources/images/icon4.png" style="cursor:pointer;" height="50%;" width="95%;">
                                                    <input id="headurladd" type="file" name="headUrl1" onchange="readFile(this)"  "/>
                                                        <%--style="display: none;"--%>
                                                    <input type="hidden" name="headUrl" id="logo" value="" />
                                                </form>
                                                <%--下载弹出框 一个a标签就行！！！--%>
                                                <%--<a href="http://l|ocalhost:8080/api/ops/accountInformationRestApi/exportPersonalList?format=json&sid=825fff31-3ca5-41ea-8895-6b4dc37e33c1&location='上海市'"/>--%>
                                                <form id="testCookieForm" method="post"  action="http://localhost:8080/login?format=json" >
                                                    principal <input type="text"  name="principal"/>
                                                    credentials <input type="text"  name="credentials"/>
                                                    <input type="submit" value = "上传">
                                                    <%--<input type="submit" value="上传">--%>
                                                </form>
                                            </div>
...
//    formData 对象会把表单中所有input标签 以 name   value的形式封装后传到后台
//    后台可以用MultipartFile接收
//    文件上传
    function fileUpload(){
       var formData = new FormData($("#uploadForm")[0]);
        alert(formData);
        $.ajax({
            url:"http://localhost:8080/api/zw/userHandApi/imgUpload",
            type:"post",
            data:formData,
            processData:false,
            contentType:false,
